...
export const StockList = () => {
    ...
    const renderIcon = (change) =>{
        return change>0 ?<BsFillArrowUpSquareFill/>: <BsFillArrowDownSquareFill/>
    }
    useEffect(() => {
        let isMounted = true;
                    ...
                    stock.map((stockData) => {
                        return (
                            <tr style = {{cursor: "pointer"}} className="table-row"  key= {stockData.symbol} onClick={() => handleStockSelect(stockData.symbol)}>
                                ...
                                <td>{stockData.data.pc}<button className='btn btn-danger btn-sm ml-3 mx-2 d-inline-block delete-button' onClick={(e) => {e.stopPropagation() ;deleteStock(stockData.symbol)}}>Remove</button></td>
                            </tr>
                        )
                    })
                }
